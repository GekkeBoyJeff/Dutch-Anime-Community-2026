			...
			<div className="badge-grid">
				...
				{badges.map((b) => (
					<article key={b.id} className="badge-card">
						...
						{b.image_path && <img className="badge-img" src={imageUrl(b.image_path)} alt="" width={64} height={64} />}
						<span className="badge-title">{b.title}</span>
						{b.description && <span className="con-note">{b.description}</span>}
						<span className="con-note">{formatDate(b.awarded_on, { dateStyle: 'medium' }) ?? b.awarded_on}</span>
						{canManage && (
							<Button variant="ghost" onClick={() => setToArchive(b)}>
								Intrekken
							</Button>
						)}
					</article>
