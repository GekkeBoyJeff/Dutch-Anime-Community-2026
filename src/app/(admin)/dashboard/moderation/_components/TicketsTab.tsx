...
import TicketUpload from '@/app/(admin)/dashboard/moderation/_components/TicketUpload';
import TicketViewer from '@/app/(admin)/dashboard/moderation/_components/TicketViewer';
...
const TicketsTab = ({ subjectId, sessionUserId, canManage, canDelete }: Props) => {
			...
			{ key: 'status', header: 'Status', align: 'center', cell: (r) => (r.archived_at ? <StatusBadge domain="request" status="cancelled" label="Gearchiveerd" /> : '—') },
			{
				...
				cell: (r) => (
					<span className="inventory-row-actions">
						<Button variant="secondary" onClick={() => setViewerId(r.id)}>
							Bekijk
						</Button>
						{canManage && !r.archived_at && (
							<Button variant="ghost" onClick={() => setToArchive(r)}>
								Archiveer
							</Button>
						)}
						{canDelete && (
	...
	return (
		<div className="inventory-tab">
			...
			<DataTable columns={columns} data={rows} empty={{ title: 'Geen tickets', description: 'Er zijn nog geen transcripts aan dit profiel gekoppeld.' }} />
