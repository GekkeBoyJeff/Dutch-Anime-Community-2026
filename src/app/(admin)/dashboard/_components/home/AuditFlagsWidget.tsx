...
const AuditFlagsWidget = ({ session: _session }: WidgetProps) => {
	...
	return (
		<WidgetShell title="Recente wijzigingen" href="/dashboard/logs" linkLabel="Naar logs" loading={loading} error={error} isEmpty={!data} hideWhenEmpty>
			{data && (
				<ul className="widget-list">
					{data.map((row) => (
						<li key={row.id} className="widget-row">
							<span className="widget-row-info">
								<span className="widget-row-main">{labelOf(row)}</span>
								<span className="widget-row-sub">
									{humanizeTable(row.table_name)} · {fmt(row.created_at)}
								</span>
							</span>
							<Badge variant={OP_META[row.op]?.variant ?? 'neutral'}>{OP_META[row.op]?.label ?? row.op}</Badge>
						</li>
					))}
				</ul>
			)}
