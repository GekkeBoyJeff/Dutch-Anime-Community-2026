...
const EventDetail = ({
	...
	const assignmentRows: ReactNode[][] = assignments.map((a) => [
		itemName(a.item_id),
		personName(a.assigned_user_id, a.assigned_label),
		String(a.quantity),
		a.expected_to_bring ? 'Ja' : 'Nee',
		a.notes ?? '—',
		<Button key="d" variant="ghost" icon="trash" onClick={() => deleteAssignment(a.id)}>Verwijder</Button>,
	]);

	const ticketRows: ReactNode[][] = tickets.map((t) => [
		t.day ?? '—',
		personName(t.assigned_user_id, t.assigned_label),
		String(t.quantity),
		t.ticket_pdf_path ? 'PDF' : t.note ?? 'Wordt per bandje op de dag zelf geregeld',
		<Button key="d" variant="ghost" icon="trash" onClick={() => deleteTicket(t.id)}>Verwijder</Button>,
	]);

	return (
		<section className="inventory-section">
			<div className="inventory-toolbar">
				<Title size={4}>Beheer: {event.name}</Title>
				<Button variant="secondary" icon="close" onClick={onClose}>Sluiten</Button>
			</div>

			<div className="inventory-toolbar">
				<Title size={5}>Toegewezen items (mee te nemen)</Title>
				<Button variant="primary" icon="plus" onClick={() => setAssignForm(EMPTY_ASSIGN(items[0]?.id ?? ''))}>Item toewijzen</Button>
			</div>
			<Table columns={[{ header: 'Item' }, { header: 'Persoon' }, { header: 'Aantal', align: 'center' }, { header: 'Meenemen', align: 'center' }, { header: 'Notitie' }, { header: '' }]} rows={assignmentRows} />

			<div className="inventory-toolbar">
				<Title size={5}>Tickets</Title>
				<Button variant="primary" icon="plus" onClick={() => setTicketForm({ ...EMPTY_TICKET })}>Ticket toevoegen</Button>
			</div>
