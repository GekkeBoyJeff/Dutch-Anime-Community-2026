...
export const SearchPaletteOverlayProps = SearchPaletteProps.extend({
	onClose: z.custom<() => void>().optional().describe('Fires when the palette requests to close (Escape, backdrop, after a select)'),
}).meta({ title: 'SearchPaletteOverlay' });
export type SearchPaletteOverlayProps = z.infer<typeof SearchPaletteOverlayProps>;
