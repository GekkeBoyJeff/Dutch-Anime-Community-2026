...
export const TitleProps = z
	.object({
		...
		href: z.string().optional().describe('Wraps the text in a link to this URL'),
		linkClassName: z.string().optional().describe('Additional classes on that link'),
		id: z.string().optional().describe('Id on the root element; Base UI injects one when this is cloned through `render=`, and points the popup aria-labelledby at it'),
	})
	.meta({ title: 'Title' });
