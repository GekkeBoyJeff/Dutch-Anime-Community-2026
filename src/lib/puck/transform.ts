...
/** Deterministic change check: publish only writes the structures row when the edited chrome differs
 * from what was loaded, so an unchanged publish leaves overlays untouched. Key order never causes a
 * false positive (see stableStringify). */
