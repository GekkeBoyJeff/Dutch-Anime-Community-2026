...
// Intl.NumberFormatOptions with its `style` key spelled `format`.
export const NumberFieldFormat = z
	.object({
		localeMatcher: z.enum(['lookup', 'best fit']).optional().describe('Locale matching algorithm to use'),
		format: z.enum(['decimal', 'percent', 'currency', 'unit']).optional().describe('The formatting style to use'),
...
export const NumberFieldProps = z
	.object({
		...
		onValueChange: z.custom<(value: number | null) => void>().optional().describe('Fires on every change with the new value (null when cleared)'),
		onValueCommitted: z.custom<(value: number | null) => void>().optional().describe('Fires once the value settles (blur after typing, pointer release)'),
		...
		ref: z.custom<Ref<HTMLInputElement>>().optional().describe('Ref to the input element'),
	})
