...
// First letters of the display name, for the avatar fallback (mod subjects have no avatar image).
...
const ProfileList = ({ canManage, onOpen }: Props) => {
	...
	// Seed the filter from ?q= once (the ⌘K palette's people search deep-links here).
	const [search, setSearch] = useState(() => searchParams.get('q') ?? '');
	const [filter, setFilter] = useState('');
	const [newForm, setNewForm] = useState<{ discord_id: string; discord_name: string } | null>(null);
	const [busy, setBusy] = useState(false);
	const [skeletonCount] = useState(() => recallRowCount('moderation', 8));
	...
	return (
		<div className="inventory-tab">
			...
			{loading ? (
				...
				<EmptyState icon="users" title="Geen profielen" description="Pas je zoekopdracht aan of maak een schaduwprofiel." />
			) : (
				<div className="mod-list reveal">{filtered.map(renderRow)}</div>
			)}
