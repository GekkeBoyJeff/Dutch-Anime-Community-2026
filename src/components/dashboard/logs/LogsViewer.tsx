...
// Mirror each tab's columns so the loading placeholder reserves the same table shape.
const ACTIVITY_SKELETON_COLUMNS = [{ header: 'Wanneer' }, { header: 'Wat' }, { header: 'Door' }];
const AUDIT_SKELETON_COLUMNS = [{ header: 'Wanneer' }, { header: 'Wat' }, { header: 'Tabel' }, { header: 'Actie', align: 'center' as const }, { header: 'Door' }, { header: '', align: 'end' as const }];
...
const LogsViewer = () => {
	...
	useEffect(() => {
		...
		Promise.all([
			...
			if (!active) return;
			const activityRows = (act ?? []) as ActivityRow[];
			const auditRows = (aud ?? []) as AuditRow[];
			setActivity(activityRows);
			setAudit(auditRows);
			rememberRowCount('logs-activity', activityRows.length);
			rememberRowCount('logs-audit', auditRows.length);
	...
	// Both tabs load from the same fetch, so one flag covers them.
	...
	const activityPanel = (
		<div className="logs-tab">
			...
			/>
			{loading ? (
				<DataTableSkeleton columns={ACTIVITY_SKELETON_COLUMNS} storageKey="logs-activity" />
			) : (
	...
	const auditPanel = (
		<div className="logs-tab">
			...
			/>
			{loading ? (
				<DataTableSkeleton columns={AUDIT_SKELETON_COLUMNS} storageKey="logs-audit" />
			) : (
				<div className="media-reveal">
	...
	return (
		<Container className="logs-page">
			...
			<Drawer
				...
				size="28rem"
			>
				{auditDetail && <AuditDetailList entries={detailEntries(auditDetail)} />}
			</Drawer>
		</Container>
	);
};
