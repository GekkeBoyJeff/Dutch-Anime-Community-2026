...
import SurveysTable, { surveyStatusOf, type SurveyRow } from '@/components/dashboard/components/SurveysTable';
...
const SurveysManager = () => {
	...
	// Deep-link from the ⌘K palette (/dashboard/surveys?new=1): open the editor on a blank survey.
	...
	// Strip ?new=1 after the editor opened above, so a refresh doesn't reopen it.
	useEffect(() => {
		if (searchParams.get('new') === '1') router.replace('/dashboard/surveys');
	}, [searchParams, router]);
