...
// Mirrors the declaraties table's columns so the loading placeholder reserves the same table shape.
...
const MyExpenses = ({ session }: { session: Session }) => {
	...
	// Deep-link from the ⌘K palette (/dashboard/expenses?new=1 → "Declareer nu"): open a blank declaratie.
	// Payout details (IBAN prefill) backfill once the load below resolves, so the deep-linked form fills in.
	const [form, setForm] = useState<ExpenseForm | null>(() => (searchParams.get('new') === '1' ? { ...EMPTY } : null));
	...
	useEffect(() => {
		...
		Promise.all([
			...
			rememberRowCount('my-expenses', expenseRows.length);
			setEvents((eventRows ?? []) as EventOpt[]);
			setPayout({ iban: payoutRow?.iban ?? '', account_holder: payoutRow?.account_holder ?? '' });
			// Backfill the deep-linked blank form's payout prefill once it loads (untouched new-form only).
			setForm((f) => (f && !f.id && !f.iban && !f.description && !f.amount ? { ...f, iban: payoutRow?.iban ?? '', account_holder: payoutRow?.account_holder ?? '' } : f));
		});
		return () => {
			active = false;
		};
	...
	// Strip ?new=1 after the form opened above, so a refresh doesn't reopen it.
	useEffect(() => {
		if (searchParams.get('new') === '1') router.replace('/dashboard/expenses');
	}, [searchParams, router]);
	...
	return (
		<div className="inventory-tab">
			...
			{expenses === null ? (
				<DataTableSkeleton columns={SKELETON_COLUMNS} storageKey="my-expenses" />
			) : (
