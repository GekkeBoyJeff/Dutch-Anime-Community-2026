...
**`contentBlocks/` (24)** — hele paginasecties; krijgen alle data via props (fetchen nooit zelf), dragen vaak een `colorset`. Elk bestand hier is een **insertable blok**: precies de 23 leden van de `Block`-unie, plus `Blocks` (S) als **dispatcher** (block-`type` → renderer). Gedeelde bouwstenen die géén blok zijn wonen in `components/`. O.a. `Hero`, `TitleText`, `TextMedia`, `Prose`, `CTABanner`, `FeatureCards`, `HighlightCards`, `IntroGrid`, `BentoGrid`, `ProfileCards`, `Reviews`, `PhotoMosaic`, `LogoCloud`, `StatBand`, `Steps`, `FaqAccordion`, `SpotlightQuote`, `Showreel`, `EventTeaser` (alle S), `StickyShowcase`, en `SubscribeToNewsletter` (S) dat het client-eiland `NewsletterForm` uit `components/` inbedt. **`CardGrid`** (C) is de enige kaartenraster-blok: `variant` kiest artikel-, event- of link-kaarten en bepaalt welke velden elk item vraagt; het filteren, zoeken, sorteren en pagineren komt uit de gedeelde `ItemCardGrid` in `components/`.

**`forms/` (15)** — form-field-primitives, gethemede wrappers over Base UI **Field/Form**. `Field` (C) bezit alle a11y-plumbing (id, `htmlFor`, `aria-describedby`, `aria-invalid`); losse controls pikken dat op binnen een `<Field>`. `FieldGroup` (S, layout + container-query-context). Verder `FieldSet`/`FieldLegend`/`FieldLabel`/`FieldDescription`/`FieldError`, en controls `TextInput`/`TextArea`/`Select`/`Checkbox`/`CheckboxGroup`/`Radio`/`FileUpload` (alle C), plus `Form` (C, react-hook-form wrapper). De barrel exporteert ook types (`RadioOption`, `SelectOption`, …).
